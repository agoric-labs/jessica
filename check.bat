...
:; test -f esm/emit-c.mjs || { npm install && npm run build; }
:; for lang in $langs; do
:;  echo "==== Checking for $lang support"
:;  "$thisdir/lang/$lang/supported.bat" || continue
:;  for emitter in "$thisdir"/esm/emit-*.mjs; do
...
:;    "$thisdir/lang/$lang/jesspipe.bat" "$emitter" -- "$thisdir"/esm/*.mjs > "$out" || { status=$?; break; }
...
:;    "$thisdir/lang/$lang/jesspipe.bat" "$thisdir/esm/main-jesspipe.mjs" -- "$emitter" -- "$thisdir"/esm/*.mjs > "$out" || { status=$?; break; }
:;    diff -u "$thisdir/lang/$target/jessica.$sfx" "$out" || { status=$?; break; }
:;  done
:; done
:; exit $status
