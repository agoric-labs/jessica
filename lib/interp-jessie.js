...
import $i_justinEvaluators from './interp-justin.js';const justinEvaluators = insulate($i_justinEvaluators);
import { addBinding as $i_addBinding, BINDING_GET as $i_BINDING_GET, doEval as $i_doEval, err as $i_err,
getRef as $i_getRef, SCOPE_GET as $i_SCOPE_GET, SCOPE_SET as $i_SCOPE_SET } from './interp-utils.js';const SCOPE_SET = insulate($i_SCOPE_SET);const SCOPE_GET = insulate($i_SCOPE_GET);const getRef = insulate($i_getRef);const err = insulate($i_err);const doEval = insulate($i_doEval);const BINDING_GET = insulate($i_BINDING_GET);const addBinding = insulate($i_addBinding);
