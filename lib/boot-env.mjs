...
function bootEnv(endowments) {
    // Bootstrap a peg tag.
    const {bootPegTag, metaCompile} = bootPeg(makePeg, bootPegAst);

    // Recreate a "peg" tag using the bootstrap peg.
    console.error('created');
    const pegTag = makePeg(bootPegTag, metaCompile);
    console.error('recreated');
    ...
    const reparsedPegAst = makePeg(pegTag, (...defs) => defs);
    console.error('reparsed');
    const a = JSON.stringify(bootPegAst, undefined, '  ');
    const b = JSON.stringify(reparsedPegAst, undefined, '  ');
    if (a !== b) {
        console.error(`=== bootPegAst
${a}
=== reparsedPegAst
${b}
===`);
        throw `FATAL: bootPegAst does not match reparsedPegAst`;
    }
