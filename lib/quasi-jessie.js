...
const makeJessie = insulate((peg, justinPeg) => {
  ...
  const jessieTag = justinPeg`
    ...
    statement <-
      block
    / IF LEFT_PAREN expr RIGHT_PAREN arm ELSE elseArm      ${(_, _2, c, _3, t, _4, e) =>
                                                             ['if', c, t, e]}
    / IF LEFT_PAREN expr RIGHT_PAREN arm                   ${(_, _2, c, _3, t) => ['if', c, t]}
    / breakableStatement
    / terminator
    / IDENT COLON statement                                ${(label, _, stat) =>
                                                             ['label', label, stat]}
    ...
    / defVar EQUALS assignExpr                             ${(id, _, e) =>
                                                             ['optionalProp', id[1], id, e]}
    ...
    methodDef <-
      method
    / GET propName LEFT_PAREN RIGHT_PAREN block            ${(_, n, _2, _3, b) =>
                                                             ['getter', n, [], b]}
    / SET propName LEFT_PAREN param RIGHT_PAREN block      ${(_, n, _2, p, _3, b) =>
                                                             ['setter', n, [p], b]};
    ...
    / defImport _COMMA STAR AS defImport                   ${(d, _, _2, d2) =>
                                                             ['importBind',
                                                              [['as', 'default', d[1]],
                                                               ['as', '*', d2[1]]]]}
    / defImport _COMMA namedImports                        ${(d, n) =>
                                                             ['importBind',
                                                              [['as', 'default', d[1]], ...n]]}
    / defImport                                            ${d =>
                                                             ['importBind',
    ...
    importDecl <-
      IMPORT importClause FROM STRING SEMI                 ${(_, v, _2, s, _3) =>
                                                             ['import', v, JSON.parse(s)]}
