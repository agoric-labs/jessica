// import { insulate } from '@agoric/jessie'; // A lot of this code is lifted from:
// https://github.com/erights/quasiParserGenerator/tree/master/src/bootbnf.js

/// <reference path="peg.d.ts"/>
import * as agjessie from '@agoric/jessie';
import { slog as $i_slog } from '@michaelfig/slog';

import $i_indent from './indent.js';

const insulate = x=>x;
const indent = insulate($i_indent);

const slog = insulate($i_slog);
...
const bootPeg = insulate((makePeg, bootPegAst) => {
  ...
  function metaCompile(baseRules) {
    ...
    const makeParserTrait = confineExpr(parserTraitMakerSrc, {
      DONE,
      EAT,
      ERROR,
      FAIL,
