...
function makeBond(computedGet, callMethod) {
    const _bonded = makeWeakMap(), _bondedNullThis = makeWeakMap();
    ...
    // return a fresh arrow function bound to null,
    ...
    const bond = (maybeThis, index) => {
        ...
        if (index === undefined) {
            // Cache for null `this` value.
            actualThis = null;
            bondedForThis = _bondedNullThis;
        }
        else {
        ...
        const bondedMethod = harden((...args) =>
            callMethod(actualThis, maybeMethod, ...args));
