import { insulate } from '@agoric/jessie'; import $i_jsonEvaluators from './interp-json.js';const jsonEvaluators = insulate($i_jsonEvaluators);
import { BINDING_GET as $i_BINDING_GET, doEval as $i_doEval, err as $i_err,
getRef as $i_getRef, SCOPE_GET as $i_SCOPE_GET } from './interp-utils.js';const SCOPE_GET = insulate($i_SCOPE_GET);const getRef = insulate($i_getRef);const err = insulate($i_err);const doEval = insulate($i_doEval);const BINDING_GET = insulate($i_BINDING_GET);
import { qrepack as $i_qrepack } from './quasi-utils.js';const qrepack = insulate($i_qrepack);
