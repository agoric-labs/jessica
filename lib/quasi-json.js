import { insulate } from '@agoric/jessie'; // Subsets of JavaScript, starting from the grammar
// as defined at
...
pureArray <-
  LEFT_BRACKET pureExpr ** _COMMA _COMMA? RIGHT_BRACKET    ${(_, es, _2, _3) => ['array', es]};

array <-
  LEFT_BRACKET element ** _COMMA _COMMA? RIGHT_BRACKET     ${(_, es, _2, _3) => ['array', es]};

# to be extended
element <- assignExpr;

# The JavaScript and JSON grammars call records "objects"

pureRecord <-
  LEFT_BRACE purePropDef ** _COMMA _COMMA? RIGHT_BRACE     ${(_, ps, _2, _3) => ['record', ps]};

record <-
  LEFT_BRACE propDef ** _COMMA _COMMA? RIGHT_BRACE         ${(_, ps, _2, _3) => ['record', ps]};
