...
function bootPeg(makePeg, bootPegAst) {
    ...
    if ((value = ${vSrc}) === FAIL) pos = ${posSrc};`;
            ...
            pred(hole) {
                return `if (!act_${hole}(self, pos)) value = FAIL;`;
            },
            act(seq, hole) {
    ...
    if (value.length === 0) value = FAIL;`;
            ...
            'super'(ident) {
                return `[pos, value] = RUN(BaseParser.rule_${ident}, pos, ${
            ...
            begin() {
                ...
                // Use the specified beginPos.
                return `value = "FIXME: something"`;
            },
            cls(cs) {
                // FIXME: interpret character ranges.
                return indent`
    console.error('Would get range', ${JSON.stringify(cs)});
                `;
            },
            dot() {
                return indent`
    [pos, value] = (1,self.rule_EAT)(pos);
                `;
            },
            lit(str) {
                const strSrc = nextVar('str');
                const iSrc = nextVar('i');
                return indent`
    [pos, value] = (1,self.rule_EAT)(pos, ${JSON.stringify(str)});
    `;
            },
            peek(patt) {
                // for backtracking
    ...
    const defaultBaseGrammar = harden({
        Parser: {
            rule_EAT: harden((pos, str) => {
                if (str === undefined) {
                    console.error('Would eat one char');
                }
                else {
                    console.error('would eat', JSON.stringify(str));
                }
                return [pos, FAIL];
            }),
        },
    ...
    console.log('\n\n' + parserTraitMakerSrc + '\n\n');
        const makeParserTrait = confine(parserTraitMakerSrc, {
            FAIL,
            ERROR: harden(() => {throw `Syntax error!`}),
            RUN: harden((rule, pos, ident) => rule(pos)),
        });
