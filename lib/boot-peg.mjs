...
function bootPeg(makePeg, bootPegAst) {
    ...
    if (value.length === 0) value = FAIL;`;
            ...
            'super'(ident) {
                return `[pos, value] = RUN(self, BaseParser.rule_${ident}, pos, ${JSON.stringify(`super.${ident}`)});`;
            },
            // PEG extensions.
            begin() {
    ...
    function quasiMemo(quasiCurry) {
        ...
        let debug = false;
        return function templateTag(templateOrDebug, ...subs) {
    ...
    function metaCompile(baseRules) {
        ...
        return function (...baseActions) {
            ...
            return Object.assign(quasiParser, {
                _asExtending,
                extends: (baseQuasiParser) => (template, ...subs) => quasiParser(template, ...subs)._asExtending(baseQuasiParser),
            });
        };
    }
    ...
    const bootPegActions = makePeg(actionExtractorTag, (_defs) => { });
    ...
    const astExtractorTag = makePeg(bootPegTag, (defs) => defs);
...
//# sourceMappingURL=boot-peg.mjs.js.map