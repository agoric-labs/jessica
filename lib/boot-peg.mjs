...
function unescape(cs) {
    ...
    default:
        let i = 1;
        let c = "0123".indexOf(cs[i]);
        if (c >= 0) {
            // It's an octal escape, so consume a byte.
            let ord = c;
            if (c > 0) {
                c = "01234567".indexOf(cs[i + 1]);
                if (c >= 0) {
...
function bootPeg(makePeg, bootPegAst) {
    ...
    const defaultBaseGrammar = quasifyParser({});
    ...
    const pegExtractorTag = (_template, ...actions) => ({defs: bootPegAst, actions});

    // Add our primitives.
    pegExtractorTag.ACCEPT = ACCEPT;
    pegExtractorTag.HOLE = HOLE;
    pegExtractorTag.FAIL = FAIL;

    const {defs, actions} = makePeg(pegExtractorTag, metaCompile);
    const pegTag = metaCompile(defs)(...actions);

    return harden({ pegExtractorTag, pegTag, metaCompile });
}
