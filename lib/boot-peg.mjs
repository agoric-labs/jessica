...
function bootPeg(makePeg, bootPegAst) {
      ...
      const self = {...BaseParser,
        EAT: harden((pos, str) => {
            ...
            if (Array.isArray(found)) {
                const segment = template[found[0]];
    ...
    if ((value = ${vSrc}) === FAIL) pos = ${posSrc};`;
            ...
            pred(hole) {
                return `if (!act_${hole}(pos)) value = FAIL;`;
            },
            act(seq, hole) {
                ...
                while (i < cs.length) {
                    ...
                    if (cs[i] === '-') {
                        ...
                        for (let k = min; k <= max; k++) {
                            classStr += String.fromCharCode(c);
                        }
                    }
                    else {
                        classStr += c;
    ...
    [pos, value] = EAT(self);
    if (value !== FAIL) {
        value = ${JSON.stringify(classStr)}.indexOf(value) ${op} 0 ? FAIL : value;
    }
                `;
            },
            dot() {
    ...
    // console.log('\n\n' + parserTraitMakerSrc + '\n\n');
        ...
        const makeParserTrait = confine(parserTraitMakerSrc, {
            FAIL,
            EAT: harden((self, pos, str) => (1,self).EAT(pos, str)),
            ERROR: harden(() => {throw Error(`Syntax error!`)}),
