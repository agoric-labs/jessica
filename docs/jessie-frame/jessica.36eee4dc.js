...
(function (global, factory) {
  ...
  function makeHardener(initialFringe, options = {}) {
    ...
    const forEach = uncurryThis(Array.prototype.forEach);
    const TError = TypeError;
    ...
    function harden(root) {
      ...
      function enqueue(val, path) {
        ...
        if (type !== 'object' && type !== 'function') {
          ...
          throw new TError(`Unexpected typeof: ${type}`);
        }
      ...
      function checkPrototypes() {
        prototypes.forEach((path, p) => {
          if (!(toFreeze.has(p) || fringeSet.has(p))) {
            ...
            msg += ` is not already in the fringeSet`;
            if (failedString) {
              msg += ` (and .toString failed)`;
              try {
                console.log(msg);
...
},{}],"iDNi":[function(require,module,exports) {
...
function safeStringifyFunction(fn) {
  let src = `'use strict'; (${fn})`; // esm module creates "runtime" as "_" + hex(3) + "\u200D"
  // Restore eval which is modified by esm module.
  // (0, eval) => (0, <runtime>.e)

  src = src.replace(/\(0,\s*_[0-9a-fA-F]{3}\u200D\.e\)/g, '(0, eval)'); // Restore globals such as Reflect which are modified by esm module.
  // Reflect => <runtime>.e.Reflect

  src = src.replace(/_[0-9a-fA-F]{3}\u200D\.g\./g, ''); // Remove code coverage which is injected by nyc module.
...
function buildCallAndWrapError() {
  ...
  const errorNameToErrorConstructor = new Map([['EvalError', EvalError], ['RangeError', RangeError], ['ReferenceError', ReferenceError], ['SyntaxError', SyntaxError], ['TypeError', TypeError], ['URIError', URIError]]);
  const errorConstructors = new Set([EvalError.prototype, RangeError.prototype, ReferenceError.prototype, SyntaxError.prototype, TypeError.prototype, URIError.prototype, Error.prototype]);
  ...
  function callAndWrapError(target, args) {
    try {
      ...
      try {
        ...
        eStack = `${err.stack || eMessage}`; // eName/eMessage/eStack are now realm-independent primitive strings, and
        // safe to expose.
      } catch (ignored) {
...
const buildCallAndWrapErrorString = safeStringifyFunction(buildCallAndWrapError); // Declare shorthand functions. Sharing these declarations across modules
...
const {
  ...
  freeze,
  defineProperties,
  // Object.defineProperty is allowed to fail
...
const objectHasOwnProperty = uncurryThis(Object.prototype.hasOwnProperty),
      ...
      stringIncludes = uncurryThis(String.prototype.includes); // These value properties of the global object are non-writable,
// non-configurable data properties.

const frozenGlobalPropertyNames = [// *** 18.1 Value Properties of the Global Object
'Infinity', 'NaN', 'undefined']; // All the following stdlib items have the same name on both our intrinsics
...
const stableGlobalPropertyNames = [// *** 18.2 Function Properties of the Global Object
...
'Array', 'ArrayBuffer', 'Boolean', 'DataView', // 'Date',  // Unstable
// 'Error',  // Unstable
'EvalError', 'Float32Array', 'Float64Array', // 'Function',  // comes from safeFunction instead
'Int8Array', 'Int16Array', 'Int32Array', 'Map', 'Number', 'Object', // 'Promise',  // Unstable
// 'Proxy',  // Unstable
'RangeError', 'ReferenceError', // 'RegExp',  // Unstable
'Set', // 'SharedArrayBuffer'  // removed on Jan 5, 2018
...
function getSharedGlobalDescs(unsafeGlobal) {
  ...
  describe(frozenGlobalPropertyNames, false, false, false); // The following is correct but expensive.
  ...
  // FIXME: was: describe(stableGlobalPropertyNames, false, false, false);

  describe(stableGlobalPropertyNames, true, false, true); // These we keep replaceable and removable, because we expect
...
function repairAccessors() {
  ...
  } = Object; // On some platforms, the implementation of these functions act as
...
const repairFunctionsString = safeStringifyFunction(repairFunctions); // Create a new unsafeRec from a brand new context, with new intrinsics and a
// new global object
...
function getOptimizableGlobals(globalObject, localObject = {}) {
  const globalNames = getOwnPropertyNames(globalObject); // getOwnPropertyNames does ignore Symbols so we don't need this extra check:
  // typeof name === 'string' &&
...
function buildScopeHandler(unsafeRec, safeGlobal, endowments = {}, sloppyGlobals = false) {
  const {
    unsafeGlobal,
    unsafeEval
  } = unsafeRec;
...
function buildSafeEval(unsafeRec, safeEvalOperation) {
  ...
  const {
    defineProperties
  } = Object; // We use the the concise method syntax to create an eval without a
  ...
  }.eval; // safeEval's prototype RootRealm's value and instanceof Function
...
function applyTransforms(rewriterState, transforms, powers = undefined) {
  const {
    create,
    getOwnPropertyDescriptors
  } = powers || Object;
  ...
  const arrayReduce = uncurryThis(Array.prototype.reduce); // Clone before calling transforms.
  ...
  rewriterState = arrayReduce(transforms, (rs, transform) => transform.rewrite ? transform.rewrite(rs) : rs, rewriterState); // Clone after transforms
...
const applyTransformsString = safeStringifyFunction(applyTransforms); // https://www.ecma-international.org/ecma-262/9.0/index.html#sec-html-like-comments
...
function createSafeEvaluatorFactory(unsafeRec, safeGlobal, transforms, sloppyGlobals) {
  ...
  function factory(endowments = {}, options = {}) {
    ...
    function safeEvalOperation(src) {
      ...
      const scopeHandler = unsafeEval(buildScopeHandlerString)(unsafeRec, safeGlobal, rewriterState.endowments, sloppyGlobals);
...
function createFunctionEvaluator(unsafeRec, safeEvalOperation) {
  ...
  function safeFunctionOperation(...params) {
    ...
    if (!regexpTest(/^[\w\s,]*$/, functionParams)) {
      throw new SyntaxError('shim limitation: Function arg must be simple ASCII identifiers, possibly separated by commas: no default values, pattern matches, or non-ASCII parameter names'); // this protects against Matt Austin's clever attack:
...
function createRealmRec(unsafeRec, transforms, sloppyGlobals) {
  ...
  const safeEvaluatorFactory = createSafeEvaluatorFactory(unsafeRec, safeGlobal, transforms, sloppyGlobals);
...
function initRootRealm(parentUnsafeRec, self, options) {
  ...
  const Realm = unsafeEval(buildChildRealmString)(unsafeRec, // eslint-disable-next-line no-use-before-define
  BaseRealm); // Add a Realm descriptor to sharedGlobalDescs, so it can be defined onto the
  ...
  const realmRec = createRealmRec(unsafeRec, transforms, sloppyGlobals); // Apply all shims in the new RootRealm. We don't do this for compartments.
...
function initCompartment(unsafeRec, self, options = {}) {
  ...
  const realmRec = createRealmRec(unsafeRec, transforms, sloppyGlobals); // The realmRec acts as a private field on the realm instance.
...
function removeProperties(global, whitelist) {
  ...
  function clean(value, prefix, num) {
    ...
    if (proto !== null && !whiteTable.has(proto)) {
      ...
      throw new Error(`unexpected intrinsic ${prefix}.__proto__: ${proto}`);
    }

    cleaning.set(value, true);
    forEach(gopn(value), name => {
...
var whitelist = {
  ...
  namedIntrinsics: {
    ...
    // 18.2
    eval: j,
    // realms-shim depends on having indirect eval in the globals
    isFinite: t,
    isNaN: t,
    parseFloat: t,
    parseInt: t,
...
function repairDataProperties(intrinsics, repairPlan) {
  ...
  function enableDerivedOverride(obj, prop, desc) {
    if ('value' in desc && desc.configurable) {
      const {
        value
      } = desc; // eslint-disable-next-line no-inner-declarations
      ...
      getter.value = value; // eslint-disable-next-line no-inner-declarations
  ...
  function repairAllProperties(obj) {
    ...
    ownKeys(descs).forEach(prop => enableDerivedOverride(obj, prop, descs[prop]));
  }
...
function createSESWithRealmConstructor(creatorStrings, Realm) {
  function makeSESRootRealm(options) {
    ...
    const repairPlan = optDataPropertiesToRepair !== undefined ? JSON.parse(JSON.stringify(optDataPropertiesToRepair)) : dataPropertiesToRepair; // Forward the designated Realms options.
...
You probably want a Compartment instead, like:
  const c = s.global.Realm.makeCompartment({ sloppyGlobals: true })`);
    ...
    } else if (wl.namedIntrinsics.Error) {
    ...
    shims.push(removeProp); // Add options.shims.

    if (optionalShims) {
      shims.push(...optionalShims);
    }

    const r = Realm.makeRootRealm({ ...realmsOptions,
      shims
    ...
    b.createSESInThisRealm(r.global, creatorStrings, r); // Allow harden to be accessible via the SES global.
    ...
    const namedIntrinsics = r.evaluate(`(${getNamedIntrinsics})`)(r.global, wl); // Gather the intrinsics only.

    const allIntrinsics = r.evaluate(`(${getAllPrimordials$1})`)(namedIntrinsics, anonIntrinsics); // Gather the primordials and the globals.

    const allPrimordials = r.evaluate(`(${getAllPrimordials})`)(r.global, anonIntrinsics); // Repair the override mistake on the intrinsics only.

    r.evaluate(`(${repairDataProperties})`)(allIntrinsics, repairPlan); // Global this support.

    r.evaluate(`this.globalThis = this`); // The getAnonIntrinsics function might be renamed by e.g. rollup. The
    // removeProperties() function references it by name, so we need to force
    // it to have a specific name.
    ...
    */
    ...
    harden(allPrimordials); // build the makeRequire helper, glue it to the new Realm
...
},{"@agoric/harden":"KXoB"}],"pIUN":[function(require,module,exports) {
"use strict";
...
},{"@agoric/harden":"KXoB"}],"GTnL":[function(require,module,exports) {
"use strict";
...
exports.insulate = void 0;

var _confine = require("./confine.js");

var _makers = require("./makers.js");
...
},{"./confine.js":"OmBg","./makers.js":"pIUN","@agoric/harden":"KXoB"}],"g5IB":[function(require,module,exports) {
...
},{"process":"g5IB"}],"Aq2h":[function(require,module,exports) {
"use strict";
...
var _default = indent;
exports.default = _default;
},{"@agoric/jessie":"GTnL"}],"E71q":[function(require,module,exports) {
"use strict";
...
var _indent = _interopRequireDefault(require("./indent.js"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function _templateObject32() {
  var data = _taggedTemplateLiteral(["reparsedPegAst does not match src/boot-pegast.js.ts"]);
...
function _templateObject31() {
  var data = _taggedTemplateLiteral(["// boot-pegast.js.ts - AUTOMATICALLY GENERATED by boot-peg.js.ts\nexport default ", ";"], ["// boot-pegast.js.ts - AUTOMATICALLY GENERATED by boot-peg.js.ts\\nexport default ", ";"]);
...
exports.default = _default;
},{"@agoric/jessie":"GTnL","@michaelfig/slog":"Ipmo","./indent.js":"Aq2h"}],"oVyE":[function(require,module,exports) {
"use strict";
...
// boot-pegast.js.ts - AUTOMATICALLY GENERATED by boot-env.js.ts
...
exports.default = _default;
},{"@agoric/jessie":"GTnL"}],"GjQL":[function(require,module,exports) {
"use strict";
...
exports.default = _default;
},{"@agoric/jessie":"GTnL"}],"hwm9":[function(require,module,exports) {
"use strict";
...
exports.default = _default;
},{"@agoric/jessie":"GTnL"}],"xON8":[function(require,module,exports) {
"use strict";
...
exports.default = _default;
},{"@agoric/jessie":"GTnL"}],"ekd0":[function(require,module,exports) {
"use strict";
...
},{"@agoric/jessie":"GTnL"}],"Hx9y":[function(require,module,exports) {
"use strict";
...
var _quasiUtils = require("./quasi-utils.js");
...
exports.default = _default;
},{"@agoric/jessie":"GTnL","./quasi-utils.js":"ekd0"}],"FDmF":[function(require,module,exports) {
"use strict";
...
exports.default = _default;
},{"@agoric/jessie":"GTnL","@michaelfig/slog":"Ipmo"}],"QjUU":[function(require,module,exports) {
"use strict";
...
exports.default = _default;
},{"@agoric/jessie":"GTnL"}],"aBJe":[function(require,module,exports) {
"use strict";
...
var _bootPeg = _interopRequireDefault(require("./boot-peg.js"));

var _bootPegast = _interopRequireDefault(require("./boot-pegast.js"));

var _quasiPeg = _interopRequireDefault(require("./quasi-peg.js"));
...
var _quasiJustin = _interopRequireDefault(require("./quasi-justin.js"));

var _rewriteDefine = _interopRequireDefault(require("./rewrite-define.js"));

var _tagString = _interopRequireDefault(require("./tag-string.js"));
...
var _translate = require("../../lib/translate.js");
...
//# sourceMappingURL=jessica.36eee4dc.js.map